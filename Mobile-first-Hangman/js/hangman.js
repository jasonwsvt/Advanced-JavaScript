...
var app = new Vue({
	...
	methods: {
		...
		// fill this.worDivs with empty strings to create the orange blanks
		makeBlanks: function() {
			//for (let i = 0; i < this.currentWord.length; i++) {
			//	this.wordDivs.push("");
			//}
			this.currentWord.split("").forEach(letter => this.wordDivs.push(""));
		},
		...
		updateCanvas: function(ctx) {
			...
			if (this.guesses === 0) {
				ctx.beginPath();
				ctx.arc(this.canvas.width * .4, (this.canvas.height / 5) + 20, 20, 0, 2 * Math.PI);
				ctx.stroke();
				ctx.closePath();
			}
			//draw torso
